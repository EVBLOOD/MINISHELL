...
/*   Updated: 2022/04/18 02:04:31 by sakllam          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_executeappend(t_tree *tree, char **env)
{
	...
	if (id == 0)
	{
		...
		fd = open(command[0], O_WRONLY | O_APPEND | O_CREAT, 0x0201);
		dup2(fd, 1);
		close (fd);
		if (!path)
			return (printf("NO SUCH A CMD\n"));
...
int	ft_executeherdoc(t_tree *tree, char **env)
{
	...
	if (id == 0)
	{
		command = ft_dealwithlist(tree->elements, env, 0);
		path = ft_isacmd(command[1], env);
		fd = open(command[1], O_RDWR); // change values
		dup2(fd, 1);
		close (fd);
		if (!path)
			return (printf("NO SUCH A CMD\n"));
...
int	ft_executeout(t_tree *tree, char **env)
{
	...
	if (id == 0)
	{
		command = ft_dealwithlist(tree->elements, env, 0);
		path = ft_isacmd(command[1], env);
		fd = open(command[0], O_RDWR | O_CREAT | O_TRUNC, 0x0201);;
		dup2(fd, 1);
		close (fd);
		if (!path)
			return (printf("NO SUCH A CMD\n"));
...
int	ft_executereds(t_tree *tree, char **env, t_executenop arg)
{
	...
	if (command[1] == NULL)
	{
		if (arg.parrent->type == RDAPP)
			ret = open(command[0], O_RDWR | O_CREAT | O_APPEND, 0x0201);
		if (arg.parrent->type == RDOUT)
			ret = open(command[0], O_RDWR | O_CREAT | O_TRUNC, 0x0201);
	...
	if (tree->type != PIPE && tree->type != AND && tree->type != OR && tree->type != RDIN && tree->type != RDOUT && tree->type != RDAPP && tree->type != RDHER)
	{
		...
		puts("???");
			if (arg.parrent->type == AND)
				...
				return (ft_simplecmd(env, tree));
			else if (arg.parrent->type == RDAPP || arg.parrent->type == RDOUT || arg.parrent->type == RDHER || arg.parrent->type == RDIN)
